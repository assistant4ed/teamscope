    ...
    <div className="h-full overflow-y-auto bg-slate-50 p-4 md:p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
        <StatCard
          ...
          value={`${todayFiled}/${activeSubCount}`}
          sub={activeSubCount - todayFiled > 0
            ? `${activeSubCount - todayFiled} pending`
            : 'all subscribers reported'}
        />
        <StatCard
...
function ManualLogModal({ subscribers, onDone, onClose }: {
  ...
  const rosterEmpty = active.length === 0;
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/40 grid place-items-center p-4"
      ...
      <form onSubmit={save}
        ...
        <div className="p-6 space-y-3 overflow-y-auto">
          {rosterEmpty && (
            <div className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-2">
              Couldn't load the active subscriber roster. Refresh the page and try again.
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              ...
              <input type="date" value={reportDate} max={todayLocalISO()}
        ...
        <footer className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-2">
          ...
          <button type="submit" disabled={busy || rosterEmpty}
